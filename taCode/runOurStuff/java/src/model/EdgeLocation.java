...
public class EdgeLocation {
	...
	public EdgeLocation(int x, int y, Direction direction) {
		setX(x);
		setY(y);
		setDirection(direction);
	}

	public EdgeLocation(int x, int y, EdgeDirection edgeDirection) {
		setX(x);
		setY(y);
		setDirection(Direction.fromEdgeDirection(edgeDirection));
	}
	...
	public boolean validate(int radius) {
		...
		case N:
			if (edge.x > radius - 1 || edge.x < -radius) return false;
			if (edge.y > radius - 1 || edge.y < -radius) return false;
			break;
		case NW:
			if (edge.x > radius - 1 || edge.x < -radius) return false;
			if (edge.y > radius - 1 || edge.y < -radius) return false;
